defmodule LemonadeStand.Application do
  # See https://hexdocs.pm/elixir/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  @impl true
  def start(_type, _args) do
    children = [
      LemonadeStandWeb.Telemetry,
      LemonadeStand.Repo,
      {DNSCluster, query: Application.get_env(:lemonade_stand, :dns_cluster_query) || :ignore},
      {Phoenix.PubSub, name: LemonadeStand.PubSub},
      # Start a worker by calling: LemonadeStand.Worker.start_link(arg)
      # {LemonadeStand.Worker, arg},
      # Start to serve requests, typically the last entry
      LemonadeStandWeb.Endpoint,
      {AshAuthentication.Supervisor, [otp_app: :lemonade_stand]}
    ]

    # See https://hexdocs.pm/elixir/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: LemonadeStand.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  @impl true
  def config_change(changed, _new, removed) do
    LemonadeStandWeb.Endpoint.config_change(changed, removed)
    :ok
  end
end
